...
/// Interface between the TVirtualMagField and MagneticWrapperChebyshev: wrapper
...
public:
  ...
  /// Initialize the field with Geant integration option "integ" and max field
  /// "fmax", Impose scaling of parameterized L3 field by factorSol and of
  /// dipole by factorDip. The "be" is the energy of the beam in GeV/nucleon
  MagneticField(const char *name, const char *title, Double_t factorSol = 1.,
                ...
                Double_t benergy = -1, Int_t integ = 2, Double_t fmax = 15,
  ...
  void Field(const Double_t *__restrict__ point,
             Double_t *__restrict__ bField) override;
  ...
  /// Method to calculate the integral_0^z of br,bt,bz in cylindrical
  /// coordinates ( -pi<phi<pi convention )
  void getTPCIntegralCylindrical(const Double_t *rphiz, Double_t *b) const;

  /// Method to calculate the integral_0^z of bx/bz,by/bz and
  /// (bx/bz)^2+(by/bz)^2 in cylindrical coordinates ( -pi<phi<pi convention )
  ...
  /// Sets the sign/scale of the current in the L3 according to
  /// sPolarityConvention
  void setFactorSolenoid(Float_t fc = 1.);

  /// Sets the sign*scale of the current in the Dipole according to
  /// sPolarityConvention
  void setFactorDipole(Float_t fc = 1.);

  /// Returns the sign*scale of the current in the Dipole according to
  /// sPolarityConventionthe
  Double_t getFactorSolenoid() const;

  /// Return the sign*scale of the current in the Dipole according to
  /// sPolarityConventionthe
  ...
  Double_t getCurrentSolenoid() const {
    return getFactorSolenoid() *
           (mMapType == MagFieldParam::k2kG ? 12000 : 30000);
  }
  ...
  void MachineField(const Double_t *__restrict__ x,
  ...
  static MagneticField *createFieldMap(
      Float_t l3Current = -30000., Float_t diCurrent = -6000.,
      Int_t convention = 0, Bool_t uniform = kFALSE, Float_t beamenergy = 7000,
      const Char_t *btype = "pp",
      const std::string path = std::string(gSystem->Getenv("VMCWORKDIR")) +
                               std::string("/Common/maps/mfchebKGI_sym.root"));

protected:
  // not supposed to be changed during the run, set only at the initialization
  // via constructor
  void initializeMachineField(MagFieldParam::BeamType_t btype,
                              Double_t benergy);
...
private:
  std::unique_ptr<MagneticWrapperChebyshev>
      mMeasuredMap;                         //! Measured part of the field map
  ...
  MagFieldParam::BeamType_t
      mBeamType;        ///< Beam type: A-A (mBeamType=0) or p-p (mBeamType=1)
  Double_t mBeamEnergy; ///< Beam energy in GeV

  Int_t
      mDefaultIntegration; ///< Default integration method as indicated in Geant
  Int_t mPrecisionInteg;   ///< Alternative integration method, e.g. for higher
                           ///< precision
  ...
  Double_t
      mQuadrupoleGradient; ///< Gradient field for inner triplet quadrupoles
  ...
  static const Double_t sSolenoidToDipoleZ; ///< conventional Z of transition
                                            ///< from L3 to Dipole field
  static const UShort_t
      sPolarityConvention; ///< convention for the mapping of the curr.sign on
                           ///< main component sign

  MagneticField(const MagneticField &src);

  ClassDefOverride(o2::field::MagneticField,
                   3) // Class for all Alice MagField wrapper for measured data
