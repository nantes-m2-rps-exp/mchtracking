...
Chebyshev3DCalc::Chebyshev3DCalc()
    : mNumberOfCoefficients(0), mNumberOfRows(0), mNumberOfColumns(0),
      mNumberOfElementsBound2D(0), mPrecision(0),
      mNumberOfColumnsAtRow(nullptr), mColumnAtRowBeginning(nullptr),
      mCoefficientBound2D0(nullptr), mCoefficientBound2D1(nullptr),
      mCoefficients(nullptr), mTemporaryCoefficients2D(nullptr),
      mTemporaryCoefficients1D(nullptr) {}

Chebyshev3DCalc::Chebyshev3DCalc(const Chebyshev3DCalc &src)
    : TNamed(src), mNumberOfCoefficients(src.mNumberOfCoefficients),
      mNumberOfRows(src.mNumberOfRows), mNumberOfColumns(src.mNumberOfColumns),
      mNumberOfElementsBound2D(src.mNumberOfElementsBound2D),
      mPrecision(src.mPrecision), mNumberOfColumnsAtRow(nullptr),
      mColumnAtRowBeginning(nullptr), mCoefficientBound2D0(nullptr),
      mCoefficientBound2D1(nullptr), mCoefficients(nullptr),
      mTemporaryCoefficients2D(nullptr), mTemporaryCoefficients1D(nullptr) {
...
void Chebyshev3DCalc::Print(const Option_t *) const {
  ...
  printf("%d coefficients in %dx%dx%d matrix\n", mNumberOfCoefficients,
         mNumberOfRows, mNumberOfColumns, nmax3d);
}
...
Float_t Chebyshev3DCalc::evaluateDerivative(int dim, const Float_t *par) const {
  ...
  for (int id0 = mNumberOfRows; id0--;) {
    int nCLoc =
        mNumberOfColumnsAtRow[id0]; // number of significant coefs on this row
    if (!nCLoc) {
      mTemporaryCoefficients1D[id0] = 0;
      continue;
    }
    //
    int col0 = mColumnAtRowBeginning[id0]; // beginning of local column in the
                                           // 2D boundary matrix
    for (int id1 = nCLoc; id1--;) {
      ...
      if (dim == 2) {
        mTemporaryCoefficients2D[id1] = chebyshevEvaluation1Derivative(
            par[2], mCoefficients + mCoefficientBound2D1[id], ncfRC);
      } else {
        mTemporaryCoefficients2D[id1] = chebyshevEvaluation1D(
            par[2], mCoefficients + mCoefficientBound2D1[id], ncfRC);
      }
    }
    if (dim == 1) {
      mTemporaryCoefficients1D[id0] = chebyshevEvaluation1Derivative(
          par[1], mTemporaryCoefficients2D, nCLoc);
    } else {
      mTemporaryCoefficients1D[id0] =
          chebyshevEvaluation1D(par[1], mTemporaryCoefficients2D, nCLoc);
    }
  }
  return (dim == 0) ? chebyshevEvaluation1Derivative(
                          par[0], mTemporaryCoefficients1D, mNumberOfRows)
                    : chebyshevEvaluation1D(par[0], mTemporaryCoefficients1D,
                                            mNumberOfRows);
}

Float_t Chebyshev3DCalc::evaluateDerivative2(int dim1, int dim2,
                                             const Float_t *par) const {
  ...
  for (int id0 = mNumberOfRows; id0--;) {
    ...
    for (int id1 = nCLoc; id1--;) {
      ...
      if (dim1 == 2 || dim2 == 2) {
        mTemporaryCoefficients2D[id1] =
            same ? chebyshevEvaluation1Derivative2(
                       par[2], mCoefficients + mCoefficientBound2D1[id], ncfRC)
    ...
    if (dim1 == 1 || dim2 == 1) {
      mTemporaryCoefficients1D[id0] =
          same
              ? chebyshevEvaluation1Derivative2(par[1],
                                                mTemporaryCoefficients2D, nCLoc)
              : chebyshevEvaluation1Derivative(par[1], mTemporaryCoefficients2D,
                                               nCLoc);
    } else {
      mTemporaryCoefficients1D[id0] =
          chebyshevEvaluation1D(par[1], mTemporaryCoefficients2D, nCLoc);
...
void Chebyshev3DCalc::loadData(FILE *stream) {
  ...
  for (int id0 = 0; id0 < mNumberOfRows; id0++) {
    ...
    mColumnAtRowBeginning[id0] =
        mNumberOfElementsBound2D; // begining of this row in 2D boundary surface
  ...
  for (int i = 0; i < mNumberOfElementsBound2D; i++) {
    readLine(buffs,
             stream); // n.coeffs at 3-d dimension for the given column/row
