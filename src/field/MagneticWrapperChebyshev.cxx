...
MagneticWrapperChebyshev::MagneticWrapperChebyshev()
    : mNumberOfParameterizationSolenoid(0),
      ...
      mNumberOfRSegmentsSolenoid(nullptr), mSegmentIdSolenoid(nullptr),
      mMinZSolenoid(1.e6), mMaxZSolenoid(-1.e6),
      mParameterizationSolenoid(nullptr), mMaxRadiusSolenoid(0),
      mNumberOfParameterizationTPC(0), mNumberOfDistinctZSegmentsTPC(0),
      mNumberOfDistinctPSegmentsTPC(0), mNumberOfDistinctRSegmentsTPC(0),
      mCoordinatesSegmentsZTPC(nullptr), mCoordinatesSegmentsPTPC(nullptr),
      mCoordinatesSegmentsRTPC(nullptr), mBeginningOfSegmentsPTPC(nullptr),
      mNumberOfSegmentsPTPC(nullptr), mBeginningOfSegmentsRTPC(nullptr),
      mNumberOfRSegmentsTPC(nullptr), mSegmentIdTPC(nullptr), mMinZTPC(1.e6),
      mMaxZTPC(-1.e6), mParameterizationTPC(nullptr), mMaxRadiusTPC(0),
      mNumberOfParameterizationTPCRat(0), mNumberOfDistinctZSegmentsTPCRat(0),
      mNumberOfDistinctPSegmentsTPCRat(0), mNumberOfDistinctRSegmentsTPCRat(0),
      mCoordinatesSegmentsZTPCRat(nullptr),
      mCoordinatesSegmentsPTPCRat(nullptr),
      mCoordinatesSegmentsRTPCRat(nullptr),
      mBeginningOfSegmentsPTPCRat(nullptr), mNumberOfSegmentsPTPCRat(nullptr),
      mBeginningOfSegmentsRTPCRat(nullptr), mNumberOfRSegmentsTPCRat(nullptr),
      mSegmentIdTPCRat(nullptr), mMinZTPCRat(1.e6), mMaxZTPCRat(-1.e6),
      mParameterizationTPCRat(nullptr), mMaxRadiusTPCRat(0),
      mNumberOfParameterizationDipole(0), mNumberOfDistinctZSegmentsDipole(0),
      mNumberOfDistinctYSegmentsDipole(0), mNumberOfDistinctXSegmentsDipole(0),
      mCoordinatesSegmentsZDipole(nullptr),
      mCoordinatesSegmentsYDipole(nullptr),
      mCoordinatesSegmentsXDipole(nullptr),
      mBeginningOfSegmentsYDipole(nullptr), mNumberOfSegmentsYDipole(nullptr),
      mBeginningOfSegmentsXDipole(nullptr), mNumberOfSegmentsXDipole(nullptr),
      mSegmentIdDipole(nullptr), mMinDipoleZ(1.e6), mMaxDipoleZ(-1.e6),
      mParameterizationDipole(nullptr) {}

MagneticWrapperChebyshev::MagneticWrapperChebyshev(
    const MagneticWrapperChebyshev &src)
    : TNamed(src), mNumberOfParameterizationSolenoid(0),
      mNumberOfDistinctZSegmentsSolenoid(0),
      mNumberOfDistinctPSegmentsSolenoid(0),
      mNumberOfDistinctRSegmentsSolenoid(0),
      mCoordinatesSegmentsZSolenoid(nullptr),
...
void MagneticWrapperChebyshev::copyFrom(const MagneticWrapperChebyshev &src) {
  ...
  if (src.mNumberOfParameterizationSolenoid) {
    memcpy(mCoordinatesSegmentsZSolenoid =
               new Float_t[mNumberOfDistinctZSegmentsSolenoid],
           src.mCoordinatesSegmentsZSolenoid,
           sizeof(Float_t) * mNumberOfDistinctZSegmentsSolenoid);
    memcpy(mCoordinatesSegmentsPSolenoid =
               new Float_t[mNumberOfDistinctPSegmentsSolenoid],
           src.mCoordinatesSegmentsPSolenoid,
           sizeof(Float_t) * mNumberOfDistinctPSegmentsSolenoid);
    memcpy(mCoordinatesSegmentsRSolenoid =
               new Float_t[mNumberOfDistinctRSegmentsSolenoid],
           src.mCoordinatesSegmentsRSolenoid,
           sizeof(Float_t) * mNumberOfDistinctRSegmentsSolenoid);
    memcpy(mBeginningOfSegmentsPSolenoid =
               new Int_t[mNumberOfDistinctZSegmentsSolenoid],
           src.mBeginningOfSegmentsPSolenoid,
           sizeof(Int_t) * mNumberOfDistinctZSegmentsSolenoid);
    memcpy(mNumberOfSegmentsPSolenoid =
               new Int_t[mNumberOfDistinctZSegmentsSolenoid],
           src.mNumberOfSegmentsPSolenoid,
           sizeof(Int_t) * mNumberOfDistinctZSegmentsSolenoid);
    memcpy(mBeginningOfSegmentsRSolenoid =
               new Int_t[mNumberOfDistinctPSegmentsSolenoid],
           src.mBeginningOfSegmentsRSolenoid,
           sizeof(Int_t) * mNumberOfDistinctPSegmentsSolenoid);
    memcpy(mNumberOfRSegmentsSolenoid =
               new Int_t[mNumberOfDistinctPSegmentsSolenoid],
           src.mNumberOfRSegmentsSolenoid,
           sizeof(Int_t) * mNumberOfDistinctPSegmentsSolenoid);
    memcpy(mSegmentIdSolenoid = new Int_t[mNumberOfDistinctRSegmentsSolenoid],
           src.mSegmentIdSolenoid,
           sizeof(Int_t) * mNumberOfDistinctRSegmentsSolenoid);
    mParameterizationSolenoid =
        new TObjArray(mNumberOfParameterizationSolenoid);
    for (int i = 0; i < mNumberOfParameterizationSolenoid; i++) {
      mParameterizationSolenoid->AddAtAndExpand(
          new Chebyshev3D(*src.getParameterSolenoid(i)), i);
    }
  }
  ...
  if (src.mNumberOfParameterizationTPC) {
    ...
    memcpy(mNumberOfSegmentsPTPC = new Int_t[mNumberOfDistinctZSegmentsTPC],
           ...
           sizeof(Int_t) * mNumberOfDistinctZSegmentsTPC);
    memcpy(mBeginningOfSegmentsRTPC = new Int_t[mNumberOfDistinctPSegmentsTPC],
           src.mBeginningOfSegmentsRTPC,
           sizeof(Int_t) * mNumberOfDistinctPSegmentsTPC);
    memcpy(mNumberOfRSegmentsTPC = new Int_t[mNumberOfDistinctPSegmentsTPC],
           src.mNumberOfRSegmentsTPC,
           sizeof(Int_t) * mNumberOfDistinctPSegmentsTPC);
    memcpy(mSegmentIdTPC = new Int_t[mNumberOfDistinctRSegmentsTPC],
           src.mSegmentIdTPC, sizeof(Int_t) * mNumberOfDistinctRSegmentsTPC);
    mParameterizationTPC = new TObjArray(mNumberOfParameterizationTPC);
    for (int i = 0; i < mNumberOfParameterizationTPC; i++) {
      mParameterizationTPC->AddAtAndExpand(
          new Chebyshev3D(*src.getParameterTPCIntegral(i)), i);
    }
  }
  ...
  if (src.mNumberOfParameterizationTPCRat) {
    memcpy(mCoordinatesSegmentsZTPCRat =
    ...
    memcpy(mCoordinatesSegmentsPTPCRat =
    ...
    memcpy(mCoordinatesSegmentsRTPCRat =
    ...
    for (int i = 0; i < mNumberOfParameterizationTPCRat; i++) {
      mParameterizationTPCRat->AddAtAndExpand(
          new Chebyshev3D(*src.getParameterTPCRatIntegral(i)), i);
    }
  }
  ...
  if (src.mNumberOfParameterizationDipole) {
    memcpy(mCoordinatesSegmentsZDipole =
    ...
    memcpy(mCoordinatesSegmentsYDipole =
    ...
    memcpy(mCoordinatesSegmentsXDipole =
    ...
    for (int i = 0; i < mNumberOfParameterizationDipole; i++) {
      mParameterizationDipole->AddAtAndExpand(
          new Chebyshev3D(*src.getParameterDipole(i)), i);
    }
  }
}
...
void MagneticWrapperChebyshev::Clear(const Option_t *) {
  ...
  mNumberOfParameterizationSolenoid = mNumberOfDistinctZSegmentsSolenoid =
      mNumberOfDistinctPSegmentsSolenoid = mNumberOfDistinctRSegmentsSolenoid =
          0;
  mMinZSolenoid = 1e6;
  mMaxZSolenoid = -1e6;
  ...
  mNumberOfParameterizationTPC = mNumberOfDistinctZSegmentsTPC =
      mNumberOfDistinctPSegmentsTPC = mNumberOfDistinctRSegmentsTPC = 0;
  mMinZTPC = 1e6;
  mMaxZTPC = -1e6;
  mMaxRadiusTPC = 0;
  ...
  mNumberOfParameterizationTPCRat = mNumberOfDistinctZSegmentsTPCRat =
      mNumberOfDistinctPSegmentsTPCRat = mNumberOfDistinctRSegmentsTPCRat = 0;
  mMinZTPCRat = 1e6;
  mMaxZTPCRat = -1e6;
  mMaxRadiusTPCRat = 0;
  ...
  mNumberOfParameterizationDipole = mNumberOfDistinctZSegmentsDipole =
      mNumberOfDistinctYSegmentsDipole = mNumberOfDistinctXSegmentsDipole = 0;
  mMinDipoleZ = 1e6;
  mMaxDipoleZ = -1e6;
}
...
void MagneticWrapperChebyshev::Print(Option_t *) const {
  ...
  printf("Segmentation for Solenoid (%+.2f<Z<%+.2f cm | R<%.2f cm)\n",
         mMinZSolenoid, mMaxZSolenoid, mMaxRadiusSolenoid);
  ...
  printf("Segmentation for TPC field ratios integral (%+.2f<Z<%+.2f cm | "
         "R<%.2f cm)\n",
         mMinZTPCRat, mMaxZTPCRat, mMaxRadiusTPCRat);
...
Int_t MagneticWrapperChebyshev::findDipoleSegment(const Double_t *xyz) const {
  ...
  int xid, yid,
      zid = TMath::BinarySearch(mNumberOfDistinctZSegmentsDipole,
  ...
  while (true) {
    ...
    // to make sure that due to the precision problems we did not pick the next
    // Zbin
    if (!reCheck && (xyz[2] - mCoordinatesSegmentsZDipole[zid] < 3.e-5) &&
        zid &&
        !getParameterDipole(mSegmentIdDipole[xid])
             ->isInside(xyz)) { // check the previous Z bin
      zid--;
      reCheck = kTRUE;
      continue;
    }
...
Int_t MagneticWrapperChebyshev::findSolenoidSegment(const Double_t *rpz) const {
  ...
  int rid, pid,
      zid = TMath::BinarySearch(mNumberOfDistinctZSegmentsSolenoid,
  ...
  while (true) {
    ...
    // to make sure that due to the precision problems we did not pick the next
    // Zbin
    if (!reCheck && (rpz[2] - mCoordinatesSegmentsZSolenoid[zid] < 3.e-5) &&
        zid &&
        !getParameterSolenoid(mSegmentIdSolenoid[rid])
             ->isInside(rpz)) { // check the previous Z bin
      zid--;
      reCheck = kTRUE;
      continue;
    }
...
Int_t MagneticWrapperChebyshev::findTPCSegment(const Double_t *rpz) const {
  ...
  int rid, pid,
      zid = TMath::BinarySearch(mNumberOfDistinctZSegmentsTPC,
                                mCoordinatesSegmentsZTPC,
                                (Float_t)rpz[2]); // find zsegment
  ...
  while (true) {
    ...
    if (!reCheck && (rpz[2] - mCoordinatesSegmentsZTPC[zid] < 3.e-5) && zid &&
        !getParameterTPCIntegral(mSegmentIdTPC[rid])
             ->isInside(rpz)) { // check the previous Z bin
      zid--;
      reCheck = kTRUE;
      continue;
...
Int_t MagneticWrapperChebyshev::findTPCRatSegment(const Double_t *rpz) const {
  ...
  int rid, pid,
      zid = TMath::BinarySearch(mNumberOfDistinctZSegmentsTPCRat,
                                mCoordinatesSegmentsZTPCRat,
                                (Float_t)rpz[2]); // find zsegment
  ...
  while (true) {
    ...
    // to make sure that due to the precision problems we did not pick the next
    // Zbin
    if (!reCheck && (rpz[2] - mCoordinatesSegmentsZTPCRat[zid] < 3.e-5) &&
        zid &&
        !getParameterTPCRatIntegral(mSegmentIdTPCRat[rid])
             ->isInside(rpz)) { // check the previous Z bin
      zid--;
      reCheck = kTRUE;
      continue;
...
void MagneticWrapperChebyshev::getTPCIntegral(const Double_t *xyz,
  ...
  cartesianToCylindrical(xyz, rphiz);
#ifndef _BRING_TO_BOUNDARY_
  if ((rphiz[2] > getMaxZTPCIntegral() || rphiz[2] < getMinZTPCIntegral()) ||
      rphiz[0] > getMaxRTPCIntegral()) {
    for (int i = 3; i--;) {
      b[i] = 0;
    }
    return;
...
void MagneticWrapperChebyshev::getTPCRatIntegral(const Double_t *xyz,
  ...
  cartesianToCylindrical(xyz, rphiz);
#ifndef _BRING_TO_BOUNDARY_
  if ((rphiz[2] > getMaxZTPCRatIntegral() ||
       rphiz[2] < getMinZTPCRatIntegral()) ||
...
Double_t MagneticWrapperChebyshev::fieldCylindricalSolenoidBz(
    const Double_t *rphiz) const {
  int id = findSolenoidSegment(rphiz);
  if (id < 0) {
    return 0.;
  }
...
void MagneticWrapperChebyshev::getTPCIntegralCylindrical(const Double_t *rphiz,
  ...
  if (id >= mNumberOfParameterizationTPC) {
    LOG(error) << "MagneticWrapperChebyshev::getTPCIntegralCylindrical: Wrong "
                  "TPCParam segment "
               << id;
    b[0] = b[1] = b[2] = 0;
    return;
...
void MagneticWrapperChebyshev::getTPCRatIntegralCylindrical(
    const Double_t *rphiz, Double_t *b) const {
  ...
  if (id >= mNumberOfParameterizationTPCRat) {
    LOG(error) << "MagneticWrapperChebyshev::getTPCRatIntegralCylindrical: "
                  "Wrong TPCRatParam segment "
               << id;
    b[0] = b[1] = b[2] = 0;
    return;
  }
...
void checkExpected(char const *expected, TString &buffs) {
  if (!buffs.BeginsWith(expected)) {
    LOG(error) << R"(MagneticWrapperChebyshev::loadData: Expected: ")"
               << expected << R"( <name>", found ")" << buffs.Data()
               << "\"\nStop\n";
    exit(1);
  }
}
...
void MagneticWrapperChebyshev::buildTableSolenoid() {
  buildTable(mNumberOfParameterizationSolenoid, mParameterizationSolenoid,
             mNumberOfDistinctZSegmentsSolenoid,
             mNumberOfDistinctPSegmentsSolenoid,
             mNumberOfDistinctRSegmentsSolenoid, mMinZSolenoid, mMaxZSolenoid,
             &mCoordinatesSegmentsZSolenoid, &mCoordinatesSegmentsPSolenoid,
             &mCoordinatesSegmentsRSolenoid, &mBeginningOfSegmentsPSolenoid,
             &mNumberOfSegmentsPSolenoid, &mBeginningOfSegmentsRSolenoid,
             &mNumberOfRSegmentsSolenoid, &mSegmentIdSolenoid);
}

void MagneticWrapperChebyshev::buildTableDipole() {
  buildTable(mNumberOfParameterizationDipole, mParameterizationDipole,
             mNumberOfDistinctZSegmentsDipole, mNumberOfDistinctYSegmentsDipole,
             mNumberOfDistinctXSegmentsDipole, mMinDipoleZ, mMaxDipoleZ,
             &mCoordinatesSegmentsZDipole, &mCoordinatesSegmentsYDipole,
             &mCoordinatesSegmentsXDipole, &mBeginningOfSegmentsYDipole,
             &mNumberOfSegmentsYDipole, &mBeginningOfSegmentsXDipole,
             &mNumberOfSegmentsXDipole, &mSegmentIdDipole);
}

void MagneticWrapperChebyshev::buildTableTPCIntegral() {
  buildTable(mNumberOfParameterizationTPC, mParameterizationTPC,
             mNumberOfDistinctZSegmentsTPC, mNumberOfDistinctPSegmentsTPC,
             mNumberOfDistinctRSegmentsTPC, mMinZTPC, mMaxZTPC,
             &mCoordinatesSegmentsZTPC, &mCoordinatesSegmentsPTPC,
             &mCoordinatesSegmentsRTPC, &mBeginningOfSegmentsPTPC,
             &mNumberOfSegmentsPTPC, &mBeginningOfSegmentsRTPC,
             &mNumberOfRSegmentsTPC, &mSegmentIdTPC);
}

void MagneticWrapperChebyshev::buildTableTPCRatIntegral() {
  buildTable(mNumberOfParameterizationTPCRat, mParameterizationTPCRat,
             mNumberOfDistinctZSegmentsTPCRat, mNumberOfDistinctPSegmentsTPCRat,
             mNumberOfDistinctRSegmentsTPCRat, mMinZTPCRat, mMaxZTPCRat,
             &mCoordinatesSegmentsZTPCRat, &mCoordinatesSegmentsPTPCRat,
             &mCoordinatesSegmentsRTPCRat, &mBeginningOfSegmentsPTPCRat,
             &mNumberOfSegmentsPTPCRat, &mBeginningOfSegmentsRTPCRat,
...
void MagneticWrapperChebyshev::addParameterTPCIntegral(
    const Chebyshev3D *param) {
  if (!mParameterizationTPC) {
...
void MagneticWrapperChebyshev::addParameterTPCRatIntegral(
    const Chebyshev3D *param) {
  if (!mParameterizationTPCRat) {
...
void MagneticWrapperChebyshev::resetDipole() {
  ...
  mNumberOfParameterizationDipole = mNumberOfDistinctZSegmentsDipole =
      mNumberOfDistinctXSegmentsDipole = mNumberOfDistinctYSegmentsDipole = 0;
  mMinDipoleZ = 1e6;
  mMaxDipoleZ = -1e6;
}
...
void MagneticWrapperChebyshev::buildTable(
    Int_t npar, TObjArray *parArr, Int_t &nZSeg, Int_t &nYSeg, Int_t &nXSeg,
    Float_t &minZ, Float_t &maxZ, Float_t **segZ, Float_t **segY,
    Float_t **segX, Int_t **begSegY, Int_t **nSegY, Int_t **begSegX,
  ...
  for (int iz = 0; iz < nZSeg; iz++) {
    ...
    int ny = segmentDimension(&tmpSegY, parArr, npar, 1, 1, -1, 1, -1,
                              tmpSegZ[iz], tmpSegZ[iz + 1]) -
             1;
    ...
    for (int iy = 0; iy < ny; iy++) {
      ...
      int nx = segmentDimension(&tmpSegX, parArr, npar, 0, 1, -1, tmpSegY[iy],
                                tmpSegY[iy + 1], tmpSegZ[iz], tmpSegZ[iz + 1]) -
               1;
...
//
//   // create segmentation in Z
//   mNumberOfDistinctZSegmentsDipole = segmentDimension(&tmpSegZ,
...
//     printf("\nZSegment#%d  %+e : %+e\n",iz,tmpSegZ[iz],tmpSegZ[iz+1]);
//     int ny = segmentDimension(&tmpSegY, mParameterizationDipole,
//     mNumberOfParameterizationDipole, 1,
//          1,-1, 1,-1, tmpSegZ[iz],tmpSegZ[iz+1]) - 1;
//     segY.Set(ny + mNumberOfDistinctYSegmentsDipole);
//     for (int iy=0;iy<ny;iy++) segY[mNumberOfDistinctYSegmentsDipole+iy] =
//     tmpSegY[iy]; begSegYDip[iz] = mNumberOfDistinctYSegmentsDipole;
//     nsegYDip[iz] = ny;
//     printf(" Found %d YSegments, to start from %d\n",ny, begSegYDip[iz]);
//
//     // for each slice in Z and Y create segmentation in X
...
//       printf("\n   YSegment#%d  %+e : %+e\n",iy, tmpSegY[iy],tmpSegY[iy+1]);
//       int nx = segmentDimension(&tmpSegX, mParameterizationDipole,
...
//       for (int ix=0;ix<nx;ix++) segX[mNumberOfDistinctXSegmentsDipole+ix] =
//       tmpSegX[ix]; begSegXDip[isg] = mNumberOfDistinctXSegmentsDipole;
//       nsegXDip[isg] = nx;
//       printf("   Found %d XSegments, to start from %d\n",nx,
//       begSegXDip[isg]);
//
...
//   mCoordinatesSegmentsZDipole    = new
//   Float_t[mNumberOfDistinctZSegmentsDipole]; for (int
//   i=mNumberOfDistinctZSegmentsDipole;i--;) mCoordinatesSegmentsZDipole[i] =
//   tmpSegZ[i]; delete[] tmpSegZ;
//
//   mCoordinatesSegmentsYDipole    = new
//   Float_t[mNumberOfDistinctYSegmentsDipole]; mCoordinatesSegmentsXDipole    =
//   new Float_t[mNumberOfDistinctXSegmentsDipole]; mBeginningOfSegmentsYDipole
//   = new Int_t[mNumberOfDistinctZSegmentsDipole]; mNumberOfSegmentsYDipole   =
//   new Int_t[mNumberOfDistinctZSegmentsDipole]; mBeginningOfSegmentsXDipole =
//   new Int_t[mNumberOfDistinctYSegmentsDipole]; mNumberOfSegmentsXDipole   =
//   new Int_t[mNumberOfDistinctYSegmentsDipole]; mSegmentIdDipole   = new
//   Int_t[mNumberOfDistinctXSegmentsDipole];
//
//   for (int i=mNumberOfDistinctYSegmentsDipole;i--;)
//   mCoordinatesSegmentsYDipole[i] = segY[i]; for (int
//   i=mNumberOfDistinctXSegmentsDipole;i--;) mCoordinatesSegmentsXDipole[i] =
//   segX[i]; for (int i=mNumberOfDistinctZSegmentsDipole;i--;)
//   {mBeginningOfSegmentsYDipole[i] = begSegYDip[i];
// mNumberOfSegmentsYDipole[i] = nsegYDip[i];}
//   for (int i=mNumberOfDistinctYSegmentsDipole;i--;)
//   {mBeginningOfSegmentsXDipole[i] = begSegXDip[i];
// mNumberOfSegmentsXDipole[i] = nsegXDip[i];}
//   for (int i=mNumberOfDistinctXSegmentsDipole;i--;) {mSegmentIdDipole[i]   =
//   segID[i];}
// }
...
void MagneticWrapperChebyshev::saveData(const char *outfile) const {
  ...
  // Solenoid part
  fprintf(stream,
          "# Set of Chebyshev parameterizations for ALICE magnetic "
          "field\nSTART %s\n",
          GetName());
...
Int_t MagneticWrapperChebyshev::segmentDimension(float **seg,
                                                 const TObjArray *par, int npar,
                                                 int dim, float xmn, float xmx,
                                                 float ymn, float ymx,
                                                 float zmn, float zmx) {
  ...
  for (int ip = 0; ip < npar; ip++) {
    ...
    if (xmn < xmx && (cheb->getBoundMin(0) > (xmx + xmn) / 2 ||
                      cheb->getBoundMax(0) < (xmn + xmx) / 2)) {
      continue;
    }
    if (ymn < ymx && (cheb->getBoundMin(1) > (ymx + ymn) / 2 ||
                      cheb->getBoundMax(1) < (ymn + ymx) / 2)) {
      continue;
    }
    if (zmn < zmx && (cheb->getBoundMin(2) > (zmx + zmn) / 2 ||
                      cheb->getBoundMax(2) < (zmn + zmx) / 2)) {
      continue;
    }
