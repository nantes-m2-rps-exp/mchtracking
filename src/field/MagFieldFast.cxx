...
/// \brief Implementation of the fast magnetic field parametrization
/// MagFieldFast \author ruben.shahoyan@cern.ch
//
...
MagFieldFast::MagFieldFast(float factor, int nomField, const string inpFmt)
  ...
  if (nomField != 2 && nomField != 5) {
    LOG(fatal) << "No parametrization for nominal field of " << nomField
               << " kG";
  }
  TString pth;
...
bool MagFieldFast::LoadData(const string inpFName) {
  ...
  std::string line;
  int valI, component = -1, nParams = 0,
            header[4] = {-1, -1, -1, -1}; // iR, iZ, iQuadrant, nVal
  ...
  while (std::getline(in, line)) {
    ...
    if (component < 0) {
      ...
      if (cnt != header[3]) {
        LOG(fatal) << "Wrong data (npar=" << cnt << ") for param " << header[0]
  ...
  if (nParams != kNSolRRanges * kNSolZRanges * kNQuadrants) {
    LOG(fatal) << "Was expecting " << kNSolRRanges * kNSolZRanges * kNQuadrants
               << " params";
  }
  return true;
}
...
//_______________________________________________________________________
bool MagFieldFast::GetBcomp(EDim comp, const math_utils::Point3D<float> xyz,
                            double &b) const {
  // get field
  int zSeg, rSeg, quadrant;
...
//_______________________________________________________________________
bool MagFieldFast::GetSegment(float x, float y, float z, int &zSeg, int &rSeg,
                              int &quadrant) const {
