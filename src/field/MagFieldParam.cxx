...
MagFieldParam::MagFieldParam(const char *name, const char *title,
                             const char *context)
    : mMapType(k5kG), mBeamType(kNoBeamField), mDefaultIntegration(0),
      mFactorSol(0.), mFactorDip(0.), mBeamEnergy(0.), mMaxField(0.),
      mMapPath() {
...
//   // RS: is there a bug in FairParamList::fill(const Text_t* name,Text_t*
//   // value,const Int_t length)? I think the "if (l<length-1)" should be "if
//   // (l<length)"
//   char cbuff[lgt + 2];
