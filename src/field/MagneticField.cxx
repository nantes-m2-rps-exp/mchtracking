...
/// current signs and main field components in L3 (Bz) and Dipole (Bx) (in Alice
/// frame) 1) kConvMap2005: used for the field mapping in 2005 positive L3
/// current -> negative Bz positive Dip current -> positive Bx 2)
/// kConvMapDCS2008: defined by the microswitches/cabling of power converters as
/// of 2008 - 1st half 2009 positive L3  current -> positive Bz positive Dip
/// current -> positive Bx 3) kConvLHC : defined by LHC positive L3  current ->
/// positive Bz positive Dip current -> negative Bx
///
/// Note: only "negative Bz(L3) with postive Bx(Dipole)" and its inverse was
/// mapped in 2005. Hence the GRP Manager will reject the runs with the current
/// combinations (in the convention defined by the static Int_t
/// MagneticField::getPolarityConvention()) which do not lead to such field
/// polarities.
///
/// Explanation on integrals in the TPC region
/// getTPCInt(xyz,b) and getTPCRatInt(xyz,b) give integrals from point (x,y,z)
/// to point (x,y,0) (irrespectively of the z sign) of the following: TPCInt: b
/// contains int{bx}, int{by}, int{bz} TPCRatInt: b contains int{bx/bz},
/// int{by/bz}, int{(bx/bz)^2+(by/bz)^2}
///
...
/// b = b1-b2 with b1 and b2 coming from getTPCInt(xy_z1,b1) and
/// getTPCInt(xy_z2,b2)
///
...
MagneticField::MagneticField()
    : mMeasuredMap(nullptr), mFastField(nullptr), mMapType(MagFieldParam::k5kG),
      mSolenoid(0), mBeamType(MagFieldParam::kNoBeamField), mBeamEnergy(0),
      mDefaultIntegration(0), mPrecisionInteg(0),
      mMultipicativeFactorSolenoid(1.), mMultipicativeFactorDipole(1.),
      mMaxField(15), mDipoleOnOffFlag(kFALSE), mQuadrupoleGradient(0),
      mDipoleField(0), mCompensatorField2C(0), mCompensatorField1A(0),
      mCompensatorField2A(0), mParameterNames("", "") {
  /*
   * Default constructor
   */
...
MagneticField::MagneticField(const char *name, const char *title,
                             Double_t factorSol, Double_t factorDip,
                             MagFieldParam::BMap_t maptype,
                             MagFieldParam::BeamType_t bt, Double_t be,
                             Int_t integ, Double_t fmax, const std::string path)
    : mMeasuredMap(nullptr), mFastField(nullptr), mMapType(maptype),
      mSolenoid(0), mBeamType(bt), mBeamEnergy(be), mDefaultIntegration(integ),
      mPrecisionInteg(1), mMultipicativeFactorSolenoid(factorSol),
      mMultipicativeFactorDipole(factorDip), mMaxField(fmax),
      mDipoleOnOffFlag(factorDip == 0.), mQuadrupoleGradient(0),
      mDipoleField(0), mCompensatorField2C(0), mCompensatorField1A(0),
      mCompensatorField2A(0), mParameterNames("", "") {
  /*
...
MagneticField::MagneticField(const MagFieldParam &param)
    : mMeasuredMap(nullptr), mFastField(nullptr), mMapType(param.GetMapType()),
      mSolenoid(0), mBeamType(param.GetBeamType()),
      mBeamEnergy(param.GetBeamEnergy()),
      mDefaultIntegration(param.GetDefInt()), mPrecisionInteg(1),
      ...
      mDipoleOnOffFlag(param.GetFactorDip() == 0.), mQuadrupoleGradient(0),
      mDipoleField(0), mCompensatorField2C(0), mCompensatorField1A(0),
      mCompensatorField2A(0), mParameterNames("", "") {
  /*
...
MagneticField *MagneticField::createNominalField(int fld, bool uniform) {
  ...
  if (uniform) {
    ...
    case 2:
      ...
      break;
    default:
      LOG(fatal) << "Field option " << fld
                 << " is not supported, use +-2, +-5 or 0 or <int_kilogauss>U";
    };
  }
  return new o2::field::MagneticField("Maps", "Maps", fldCoeffL3, fldCoeffDip,
                                      fldType);
}
...
void MagneticField::CreateField() {
  ...
  if (mDefaultIntegration < 0 || mDefaultIntegration > 2) {
    LOG(warning) << "MagneticField::CreateField: Invalid magnetic field flag: "
                 << mDefaultIntegration << "; Helix tracking chosen instead";
  ...
  if (mBeamEnergy <= 0 && mBeamType != MagFieldParam::kNoBeamField) {
    ...
    } else if (mBeamType == MagFieldParam::kBeamTypepA ||
               mBeamType == MagFieldParam::kBeamTypeAp) {
      mBeamEnergy = 2760; // same rigitiy max PbPb energy
    }
    //
    LOG(info) << "MagneticField::CreateField: Maximim possible beam energy for "
                 "requested beam is assumed";
  }
  ...
  if (mMapType == MagFieldParam::k2kG) {
    ...
    LOG(fatal) << "MagneticField::CreateField: Unknown field identifier "
               << mMapType << " is requested\n";
  }
...
Bool_t MagneticField::loadParameterization() {
  ...
  if (mMeasuredMap) {
    LOG(fatal) << "MagneticField::loadParameterization: Field data "
               << getParameterName() << " are already loaded from "
               << getDataFileName() << "\n";
  }
  const char *fname = gSystem->ExpandPathName(getDataFileName());
  TFile *file = TFile::Open(fname);
  if (!file) {
    LOG(fatal) << "MagneticField::loadParameterization: Failed to open "
  ...
  if (!mMeasuredMap) {
    LOG(fatal) << "MagneticField::loadParameterization: Did not find field "
               << getParameterName() << " in " << fname << "%s\n";
  }
  file->Close();
  delete file;
  return kTRUE;
...
void MagneticField::Field(const Double_t *__restrict__ xyz,
  ...
  if (mMeasuredMap && xyz[2] > mMeasuredMap->getMinZ() &&
      xyz[2] < mMeasuredMap->getMaxZ()) {
...
Double_t MagneticField::getBz(const Double_t *xyz) const {
  ...
  if (mMeasuredMap && xyz[2] > mMeasuredMap->getMinZ() &&
    ...
    return (xyz[2] > sSolenoidToDipoleZ || mDipoleOnOffFlag)
               ? bz * mMultipicativeFactorSolenoid
...
MagneticField &MagneticField::operator=(const MagneticField &src) {
  ...
  if (this != &src) {
    ...
    mFastField.reset(src.mFastField ? new MagFieldFast(*src.getFastField())
                                    : nullptr);
  }
  return *this;
}
...
void MagneticField::initializeMachineField(MagFieldParam::BeamType_t btype,
  ...
  if (btype == MagFieldParam::kNoBeamField) {
    mQuadrupoleGradient = mDipoleField = mCompensatorField2C =
        mCompensatorField1A = mCompensatorField2A = 0.;
    return;
  }
  ...
  if (btype ==
      MagFieldParam::
          kBeamTypeAA /* || btype==kBeamTypepA || btype==kBeamTypeAp */) {
    rigScale *= 208. / 82.;
  }
  // Attention: in p-Pb the energy recorded in the GRP is the PROTON energy, no
  // rigidity rescaling is needed
...
void MagneticField::MachineField(const Double_t *__restrict__ x,
                                 Double_t *__restrict__ b) const {
  // ---- This is the ZDC part
  // Compansators for Alice Muon Arm Dipole
  const Double_t kBComp1CZ = 1075., kBComp1hDZ = 260. / 2.,
                 kBComp1SqR = 4.0 * 4.0;
  const Double_t kBComp2CZ = 2049., kBComp2hDZ = 153. / 2.,
                 kBComp2SqR = 4.5 * 4.5;

  const Double_t kTripQ1CZ = 2615., kTripQ1hDZ = 637. / 2.,
                 kTripQ1SqR = 3.5 * 3.5;
  const Double_t kTripQ2CZ = 3480., kTripQ2hDZ = 550. / 2.,
                 kTripQ2SqR = 3.5 * 3.5;
  const Double_t kTripQ3CZ = 4130., kTripQ3hDZ = 550. / 2.,
                 kTripQ3SqR = 3.5 * 3.5;
  const Double_t kTripQ4CZ = 5015., kTripQ4hDZ = 637. / 2.,
                 kTripQ4SqR = 3.5 * 3.5;

  const Double_t kDip1CZ = 6310.8, kDip1hDZ = 945. / 2., kDip1SqRC = 4.5 * 4.5,
                 kDip1SqRA = 3.375 * 3.375;
  const Double_t kDip2CZ = 12640.3, kDip2hDZ = 945. / 2., kDip2SqRC = 4.5 * 4.5,
                 kDip2SqRA = 3.75 * 3.75;
...
Double_t MagneticField::getFactorSolenoid() const {
  ...
  case kConvLHC:
    return -mMultipicativeFactorSolenoid;
  default:
    return mMultipicativeFactorSolenoid; //  case kConvMap2005: return
...
Double_t MagneticField::getFactorDipole() const {
  ...
  case kConvLHC:
    return -mMultipicativeFactorDipole;
  default:
    return mMultipicativeFactorDipole; //  case kConvMap2005: return
                                       //  mMultipicativeFactorDipole;
  }
}

MagneticField *MagneticField::createFieldMap(Float_t l3Cur, Float_t diCur,
                                             Int_t convention, Bool_t uniform,
                                             Float_t beamenergy,
  ...
  if (TMath::Abs((sclDip = diCur / diNominalCurrent) - 1.) > tolerance &&
      !uniform) {
    if (diCur <= zero) {
      sclDip = 0.; // some small current.. -> Dipole OFF
    } else {
      LOG(fatal) << "MagneticField::createFieldMap: Wrong dipole current ("
                 << diCur << " A)!";
    }
  }

  if (uniform) {
    // special treatment of special MC with uniform mag field (normalized to 0.5
    // T) no check for scaling/polarities are done
    ...
    if (TMath::Abs((sclL3 = l3Cur / l3NominalCurrent1) - 1.) < tolerance) {
    ...
    } else if (TMath::Abs((sclL3 = l3Cur / l3NominalCurrent2) - 1.) <
               tolerance) {
      ...
      LOG(fatal) << "MagneticField::createFieldMap: Wrong L3 current (" << l3Cur
                 << "  A)!";
    }
  }

  if (sclDip != 0 && map != MagFieldParam::k5kGUniform) {
    if ((l3Cur <= zero) || ((convention == kConvLHC && l3Pol != diPol) ||
                            (convention == kConvDCS2008 && l3Pol == diPol))) {
      LOG(fatal) << "MagneticField::createFieldMap: Wrong combination for "
                    "L3/Dipole polarities ("
                 << (l3Pol > 0 ? '+' : '-') << "/" << (diPol > 0 ? '+' : '-')
                 << ") for convention " << getPolarityConvention();
    }
  }
  ...
  char ttl[80];
  snprintf(ttl, 79, "L3: %+5d Dip: %+4d kA; %s | Polarities in %s convention",
           (int)TMath::Sign(l3Cur, float(sclL3)),
  ...
  return new MagneticField("MagneticFieldMap", ttl, sclL3, sclDip, map, btype,
                           beamenergy, 2, 10., path);
}
...
void MagneticField::Print(Option_t *opt) const {
  ...
  LOG(info) << "MagneticField::Print: Solenoid (" << getFactorSolenoid() << "*)"
            << ((mMapType == MagFieldParam::k5kG ||
                 mMapType == MagFieldParam::k5kGUniform)
                    ? 5.
                    : 2)
            << " kG, Dipole " << (mDipoleOnOffFlag ? "OFF" : "ON") << " ("
            << getFactorDipole() << ") "
            << (mMapType == MagFieldParam::k5kGUniform ? " |Constant Field!"
                                                       : "");
  if (opts.Contains("a")) {
    LOG(info) << "MagneticField::Print: Machine B fields for "
              << getBeamTypeText() << "  beam (" << mBeamEnergy
              << " GeV): QGrad: " << mQuadrupoleGradient
              << " Dipole: " << mDipoleField;
    LOG(info) << "MagneticField::Print: Uses " << getParameterName() << "  of "
              << getDataFileName();
  }
}

void MagneticField::FillParContainer() {
  throw std::runtime_error("not implemented in this lite version");
  // // fill field parameters
  // FairRun *fRun = FairRun::Instance();
  // FairRuntimeDb *rtdb = fRun->GetRuntimeDb();
  // MagFieldParam *par =
  //     static_cast<MagFieldParam *>(rtdb->getContainer("MagFieldParam"));
  // par->SetParam(this);
  // par->setChanged();
}
...
void MagneticField::AllowFastField(bool v) {
  if (v) {
    if (!mFastField) {
      mFastField = std::make_unique<MagFieldFast>(
