...
Chebyshev3D::Chebyshev3D()
    : mOutputArrayDimension(0), mPrecision(sMinimumPrecision),
      mChebyshevParameter(1), mMaxCoefficients(0),
      mTemporaryUserResults(nullptr), mTemporaryChebyshevGrid(nullptr),
      mUserFunctionName(""), mUserMacro(nullptr) {
  // Default constructor
  for (int i = 3; i--;) {
    mMinBoundaries[i] = mMaxBoundaries[i] = mBoundaryMappingScale[i] =
        mBoundaryMappingOffset[i] = mTemporaryCoefficient[i] = 0;
    mNumberOfPoints[i] = 0;
...
Chebyshev3D::Chebyshev3D(const Chebyshev3D &src)
    : TNamed(src), mOutputArrayDimension(src.mOutputArrayDimension),
      mPrecision(src.mPrecision), mChebyshevParameter(1),
      mMaxCoefficients(src.mMaxCoefficients), mTemporaryUserResults(nullptr),
      mTemporaryChebyshevGrid(nullptr),
      mUserFunctionName(src.mUserFunctionName), mUserMacro(nullptr) {
...
Chebyshev3D::Chebyshev3D(const char *inpFile)
    : mOutputArrayDimension(0), mPrecision(0), mChebyshevParameter(1),
      mMaxCoefficients(0), mTemporaryUserResults(nullptr),
      mTemporaryChebyshevGrid(nullptr), mUserFunctionName(""),
      mUserMacro(nullptr) {
  // read coefs from text file
  for (int i = 3; i--;) {
    mMinBoundaries[i] = mMaxBoundaries[i] = mBoundaryMappingScale[i] =
        mBoundaryMappingOffset[i] = 0;
    mNumberOfPoints[i] = 0;
    mTemporaryChebyshevGridOffs[i] = 0;
    mTemporaryCoefficient[i] = 0;
  }
...
Chebyshev3D::Chebyshev3D(const char *funName, int dimOut, const Float_t *bmin,
                         const Float_t *bmax, const Int_t *npoints,
                         Float_t prec, const Float_t *precD)
    : TNamed(funName, funName), mOutputArrayDimension(0),
      mPrecision(TMath::Max(sMinimumPrecision, prec)), mChebyshevParameter(1),
      mMaxCoefficients(0), mTemporaryUserResults(nullptr),
      mTemporaryChebyshevGrid(nullptr), mUserFunctionName(""),
      mUserMacro(nullptr) {
  if (dimOut < 1) {
    LOG(error) << "Chebyshev3D: Requested output dimension is "
...
#ifdef _INC_CREATION_Chebyshev3D_
Chebyshev3D::Chebyshev3D(void (*ptr)(float *, float *), int dimOut,
                         const Float_t *bmin, const Float_t *bmax,
                         const Int_t *npoints, Float_t prec,
                         const Float_t *precD)
    : mOutputArrayDimension(0), mPrecision(TMath::Max(sMinimumPrecision, prec)),
      mChebyshevParameter(1), mMaxCoefficients(0),
      mTemporaryUserResults(nullptr), mTemporaryChebyshevGrid(nullptr),
      mUserFunctionName(""), mUserMacro(nullptr) {
  if (dimOut < 1) {
...
#ifdef _INC_CREATION_Chebyshev3D_
Chebyshev3D::Chebyshev3D(void (*ptr)(float *, float *), int dimOut,
                         const Float_t *bmin, const Float_t *bmax, Float_t prec,
                         Bool_t run, const Float_t *precD)
    : mOutputArrayDimension(0), mPrecision(TMath::Max(sMinimumPrecision, prec)),
      mChebyshevParameter(1), mMaxCoefficients(0),
      mTemporaryUserResults(nullptr), mTemporaryChebyshevGrid(nullptr),
      mUserFunctionName(""), mUserMacro(nullptr) {
  if (dimOut != 3) {
    LOG(error) << "Chebyshev3D: This constructor works only for 3D fits, "
               << mOutputArrayDimension << "D fit was requested\n";
    exit(1);
  }
  if (dimOut < 1) {
    LOG(error) << "Chebyshev3D: Requested output dimension is "
...
void Chebyshev3D::Print(const Option_t *opt) const {
  ...
  printf("%s: Chebyshev parameterization for 3D->%dD function. Precision: %e\n",
         GetName(), mOutputArrayDimension, mPrecision);
  printf("Region of validity: [%+.5e:%+.5e] [%+.5e:%+.5e] [%+.5e:%+.5e]\n",
         mMinBoundaries[0], mMaxBoundaries[0], mMinBoundaries[1],
         mMaxBoundaries[1], mMinBoundaries[2], mMaxBoundaries[2]);
  TString opts = opt;
  opts.ToLower();
  if (opts.Contains("l")) {
    for (int i = 0; i < mOutputArrayDimension; i++) {
...
void Chebyshev3D::prepareBoundaries(const Float_t *bmin, const Float_t *bmax) {
  // Set and check boundaries defined by user, prepare coefficients for their
  ...
  for (int i = 3; i--;) {
    ...
    if (mBoundaryMappingScale[i] <= 0) {
      LOG(fatal) << "Boundaries for " << i
                 << "-th dimension are not increasing: " << mMinBoundaries[i]
                 << " " << mMaxBoundaries[i] << "\nStop\n";
    }
...
void Chebyshev3D::setuserFunction(const char *name) {
  ...
  tmpst = tmpst.Data() + tmpst.Last('/') +
          1; // Strip away any path preceding the macro file name
  int dot = tmpst.Last('.');
...
#ifdef _INC_CREATION_Chebyshev3D_
Int_t Chebyshev3D::calculateChebyshevCoefficients(const Float_t *funval, int np,
                                                  Float_t *outCoefs,
                                                  Float_t prec) {
...
void Chebyshev3D::defineGrid(const Int_t *npoints) {
  ...
  for (int id = 3; id--;) {
    ...
    if (mNumberOfPoints[id] < kMinPoints) {
      LOG(error) << "DefineGrid: at " << id << "-th dimension "
                 << mNumberOfPoints[id] << " point is requested, at least "
                 << kMinPoints << " is needed\nStop\n";
      exit(1);
    }
...
Int_t Chebyshev3D::chebyshevFit(int dmOut) {
  ...
  Float_t rTiny =
      0.1 * prec / Float_t(maxDim); // neglect coefficient below this threshold

  float ncals2count =
      mNumberOfPoints[2] * mNumberOfPoints[1] * mNumberOfPoints[0];
  float ncals = 0;
  float frac = 0;
  float fracStep = 0.001;

  for (int id2 = mNumberOfPoints[2]; id2--;) {
    mTemporaryCoefficient[2] =
        mTemporaryChebyshevGrid[mTemporaryChebyshevGridOffs[2] + id2];

    for (int id1 = mNumberOfPoints[1]; id1--;) {
      mTemporaryCoefficient[1] =
          mTemporaryChebyshevGrid[mTemporaryChebyshevGridOffs[1] + id1];

      for (int id0 = mNumberOfPoints[0]; id0--;) {
        mTemporaryCoefficient[0] =
            mTemporaryChebyshevGrid[mTemporaryChebyshevGridOffs[0] + id0];
        evaluateUserFunction(); // compute function values at Chebyshev roots of
                                // 0-th dimension
      ...
      int nc = calculateChebyshevCoefficients(fvals, mNumberOfPoints[0],
                                              tmpCoef1D, prec);
      for (int id0 = mNumberOfPoints[0]; id0--;) {
      ...
      if (ncmax < nc) {
        ncmax =
            nc; // max coefs to be kept in dim0 to guarantee needed precision
      }
    }
    // once each 1d slice of given 2d slice is parametrized, parametrize the
    // Cheb.coeffs
    for (int id0 = mNumberOfPoints[0]; id0--;) {
      calculateChebyshevCoefficients(tmpCoef2D + id0 * mNumberOfPoints[1],
                                     mNumberOfPoints[1], tmpCoef1D, -1);
      for (int id1 = mNumberOfPoints[1]; id1--;) {
        tmpCoef3D[id2 + mNumberOfPoints[2] * (id1 + id0 * mNumberOfPoints[1])] =
            tmpCoef1D[id1];
      }
    }
  }
  ...
  for (int id0 = mNumberOfPoints[0]; id0--;) {
    for (int id1 = mNumberOfPoints[1]; id1--;) {
      calculateChebyshevCoefficients(
          tmpCoef3D + mNumberOfPoints[2] * (id1 + id0 * mNumberOfPoints[1]),
          mNumberOfPoints[2], tmpCoef1D, -1);
      for (int id2 = mNumberOfPoints[2]; id2--;) {
        tmpCoef3D[id2 + mNumberOfPoints[2] * (id1 + id0 * mNumberOfPoints[1])] =
            tmpCoef1D[id2]; // store on place
      }
    }
  }

  // now find 2D surface which separates significant coefficients of 3D matrix
  ...
  for (int id0 = mNumberOfPoints[0]; id0--;) {
    for (int id1 = mNumberOfPoints[1]; id1--;) {
      for (int id2 = mNumberOfPoints[2]; id2--;) {
        ...
        resid -= cfa;
        tmpCoefSurf[id1 + id0 * mNumberOfPoints[1]] =
            id2 + 1; // how many coefs to keep
        break;
      }
    }
  }
  ...
  // find max significant column and fill the permanent storage for the max
  // sigificant column of each row
  ...
  for (int id0 = 0; id0 < nRows; id0++) {
    ...
    colAtRowBg[id0] =
        nElemBound2D; // begining of this row in 2D boundary surface
  ...
  // create the 2D matrix defining the boundary of significance for 3D
  // coeffs.matrix and count the number of siginifacnt coefficients
  ...
  for (int id0 = 0; id0 < nRows; id0++) {
    ...
    for (int id1 = 0; id1 < nCLoc; id1++) {
      coefBound2D0[col0 + id1] =
          tmpCoefSurf[id1 +
                      id0 * mNumberOfPoints[1]]; // number of coefs to store for
                                                 // 3-d dimension
  ...
  for (int id0 = 0; id0 < nRows; id0++) {
    ...
    for (int id1 = 0; id1 < ncLoc; id1++) {
      ...
      for (int id2 = 0; id2 < ncf2; id2++) {
        coefs[count++] = tmpCoef3D[id2 + mNumberOfPoints[2] *
                                             (id1 + id0 * mNumberOfPoints[1])];
      }
    }
  }
  ...
  //  for (int id1=0;id1<mNumberOfPoints[1];id1++) {
  //    printf("(%2d %2d) %2d
  //    |",id0,id1,tmpCoefSurf[id1+id0*mNumberOfPoints[1]]);
  //  }
  //  printf("\n");
  //}
...
void Chebyshev3D::saveData(const char *outfile, Bool_t append) const {
  // writes coefficients data to output text file, optionallt appending on the
...
void Chebyshev3D::saveData(FILE *stream) const {
  ...
  fprintf(stream,
          "\n# These are automatically generated data for the Chebyshev "
          "interpolation of 3D->%dD function\n",
          mOutputArrayDimension);
  fprintf(stream, "#\nSTART %s\n", GetName());
...
void Chebyshev3D::loadData(FILE *stream) {
  ...
  if (mOutputArrayDimension < 1) {
    LOG(fatal) << R"(Expected: '<number_of_output_dimensions>', found ")"
               << buffs.Data() << "\"\nStop\n";
  }
  ...
  if (!buffs.BeginsWith("END") || !buffs.Contains(GetName())) {
    LOG(fatal) << R"(Expected "END )" << GetName() << R"(", found ")"
               << buffs.Data() << "\".\nStop\n";
  }
}
...
void Chebyshev3D::setDimOut(const int d, const float *prec) {
  ...
  for (int i = 0; i < d; i++) {
    ...
    clc->setPrecision(prec && prec[i] > sMinimumPrecision ? prec[i]
                                                          : mPrecision);
...
TH1 *Chebyshev3D::TestRMS(int idim, int npoints, TH1 *histo) {
  // fills the difference between the original function and parameterization
  // (for idim-th component of the output) to supplied histogram. Calculations
  // are done in npoints random points. If the hostgram was not supplied, it
  // will be created. It is up to the user to delete it!
  if (!mUserMacro) {
    printf("No user function is set\n");
    return nullptr;
  }
  if (!histo) {
    histo = new TH1D(GetName(), "Control: Function - Parametrization", 100,
                     -2 * mPrecision, 2 * mPrecision);
  }
  ...
  for (int ip = npoints; ip--;) {
    ...
    for (int i = 3; i--;) {
      mTemporaryCoefficient[i] =
          mMinBoundaries[i] +
          mTemporaryCoefficient[i] * (mMaxBoundaries[i] - mMinBoundaries[i]);
    }
...
void Chebyshev3D::estimateNumberOfPoints(float prec, int gridBC[3][3],
                                         Int_t npd1, Int_t npd2, Int_t npd3) {
  ...
  for (int idim = 0; idim < 3; idim++) {
    float dimMN = mMinBoundaries[idim] +
                  sclDim[0] * (mMaxBoundaries[idim] - mMinBoundaries[idim]);
    float dimMX = mMinBoundaries[idim] +
                  sclDim[1] * (mMaxBoundaries[idim] - mMinBoundaries[idim]);

    int id1 = compDim[idim][0]; // 1st fixed dim
    int id2 = compDim[idim][1]; // 2nd fixed dim
    for (int i1 = 0; i1 < kScp; i1++) {
      xyz[id1] = mMinBoundaries[id1] +
                 kScl[i1] * (mMaxBoundaries[id1] - mMinBoundaries[id1]);
      for (int i2 = 0; i2 < kScp; i2++) {
        xyz[id2] = mMinBoundaries[id2] +
                   kScl[i2] * (mMaxBoundaries[id2] - mMinBoundaries[id2]);
        int *npt = getNcNeeded(xyz, idim, dimMN, dimMX, prec,
                               npdTst[idim]); // npoints for Bx,By,Bz
...
//   for (int idim=0;idim<3;idim++) {
//     float dimMN = mMinBoundaries[idim] +
//     sclDim[0]*(mMaxBoundaries[idim]-mMinBoundaries[idim]); float dimMX =
//     mMinBoundaries[idim] +
//     sclDim[1]*(mMaxBoundaries[idim]-mMinBoundaries[idim]);
//
...
//       int id1 = compDim[idim][0]; // 1st fixed dim
//       int id2 = compDim[idim][1]; // 2nd fixed dim
//       xyz[ id1 ] = mMinBoundaries[id1] + sclA[it]*(
//       mMaxBoundaries[id1]-mMinBoundaries[id1] ); xyz[ id2 ] =
//       mMinBoundaries[id2] + sclB[it]*(
//       mMaxBoundaries[id2]-mMinBoundaries[id2] );
//
//       int* npt = getNcNeeded(xyz,idim, dimMN,dimMX, Prec); // npoints for
//       Bx,By,Bz for (int ib=0;ib<3;ib++) if (npt[ib]>gridBC[ib][idim])
//       gridBC[ib][idim] = npt[ib];//+2;
//
//     }
//   }
// }
//
//
// int* Chebyshev3D::getNcNeeded(float xyz[3],int DimVar, float mn,float mx,
// float prec)
// {
//   // estimate needed number of chebyshev coefs for given function description
//   in DimVar dimension
...
//       mTemporaryCoefficient[DimVar] =  x/scale+offs; // map to requested
//       interval evaluateUserFunction(); for (int ib=3;ib--;)
//       gridVal[ib*kMaxPoint + i] = mTemporaryUserResults[ib];
//     }
//
//     for (int ib=0;ib<3;ib++) {
//       curNC[ib] =
//       Chebyshev3D::calculateChebyshevCoefficients(&gridVal[ib*kMaxPoint],
//       curNP,
...
int *Chebyshev3D::getNcNeeded(float xyz[3], int DimVar, float mn, float mx,
  ...
  // estimate needed number of chebyshev coefs for given function description in
  // DimVar dimension The values for two other dimensions must be set beforehand
  static int retNC[3];
  ...
  for (int i = 0; i < npCheck; i++) {
    mTemporaryCoefficient[DimVar] =
        TMath::Cos(TMath::Pi() * (i + 0.5) / npCheck) / scale +
        offs; // map to requested interval
    evaluateUserFunction();
    for (int ib = 3; ib--;) {
  ...
  for (int ib = 0; ib < 3; ib++) {
    retNC[ib] = Chebyshev3D::calculateChebyshevCoefficients(
        &gridVal[ib * npCheck], npCheck, &coefs[ib * npCheck], prec);
  }
  return retNC;
}
