...
Track::Track(const Track &track)
    : mParamAtClusters(track.mParamAtClusters), mCurrentParam(nullptr),
      mCurrentChamber(-1), mConnected(track.mConnected),
...
//__________________________________________________________________________
int Track::getNClustersInCommon(const Track &track, int stMin,
                                int stMax) const {
...
bool Track::isBetter(const Track &track) const {
  ...
  return ((nCl1 > nCl2) || ((nCl1 == nCl2) && (this->first().getTrackChi2() <
                                               track.first().getTrackChi2())));
}
...
void Track::tagRemovableClusters(uint8_t requestedStationMask,
  ...
  // count the number of clusters in each chamber and the number of chambers
  // fired on stations 4 and 5
  ...
  bool removable[10] = {false, false, false, false, false,
                        false, false, false, false, false};

  // for station 1, 2 and 3, there must be at least one cluster per requested
  // station
  for (int iCh = 0; iCh < 6; iCh += 2) {
    if (nClusters[iCh] + nClusters[iCh + 1] > 1 ||
        (requestedStationMask & (1 << (iCh / 2))) == 0) {
      removable[iCh] = removable[iCh + 1] = true;
    }
  }

  // for station 4 and 5, there must be at least one cluster per requested
  // station and at least 2 chambers fired (on the same station or not depending
  // on the requirement)
  if (nChFiredInSt45 == 4) {
    ...
    } else if (nClusters[6] + nClusters[7] > 1 ||
               (requestedStationMask & 0x8) == 0) {
      removable[6] = removable[7] = true;
    }
    if (nChFiredInSt5 == 2 && request2ChInSameSt45) {
      removable[8] = (nClusters[8] > 1);
      removable[9] = (nClusters[9] > 1);
    } else if (nClusters[8] + nClusters[9] > 1 ||
               (requestedStationMask & 0x10) == 0) {
...
TrackParam &Track::getCurrentParam() {
  /// get a reference to the current track parameters. Create dummy parameters
  /// if needed
  if (!mCurrentParam) {
...
void Track::print() const {
  /// Print the track parameters at first cluster and the Id of all associated
  /// clusters
