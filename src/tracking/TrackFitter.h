...
public:
  ...
  /// Set the vertex dispersion in y direction used for the track covariance
  /// seed
  void setBendingVertexDispersion(double ey) {
    mBendingVertexDispersion2 = ey * ey;
  }
  ...
  // private:
  void initTrack(const Cluster &cl1, const Cluster &cl2,
                 TrackParam &param) const;

private:
  void addCluster(const TrackParam &startingParam, const Cluster &cl,
                  TrackParam &param);
  ...
  static constexpr double SDefaultChamberZ[10] = {
      -526.16, -545.24, -676.4,  -695.4,  -967.5,
      -998.5,  -1276.5, -1307.5, -1406.6, -1437.6};
  /// default chamber thickness in X0 for reconstruction
  static constexpr double SChamberThicknessInX0[10] = {
      0.065, 0.065, 0.075, 0.075, 0.035, 0.035, 0.035, 0.035, 0.035, 0.035};

  double mBendingVertexDispersion2 =
      4900.; ///< vertex dispersion square (cm^2) in y direction

  bool mSmooth = false; ///< switch ON/OFF the smoother

  bool mUseChamberResolution =
      false; ///< switch between using chamber or cluster resolution
  double mChamberResolutionX2 =
      0.04; ///< chamber resolution square in x direction
  double mChamberResolutionY2 =
      0.04; ///< chamber resolution square in y direction
};
