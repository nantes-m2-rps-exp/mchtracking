...
void TrackFitter::initField(float l3Current, float dipoleCurrent) {
  ...
  auto field = o2::field::MagneticField::createFieldMap(
      l3Current, dipoleCurrent, o2::field::MagneticField::kConvLHC, false,
...
void TrackFitter::fit(
    Track &track, bool smooth, bool finalize,
  ...
  /// If finalize = true: copy the smoothed parameters, if any, into the regular
  /// ones Fit the entire track or only the part upstream itStartingParam Throw
  ...
  if (itStartingParam != nullptr) {
    ...
    // or start from the last cluster and compute the track parameters from its
    // position and the one of the first previous cluster found on a different
    // chamber
    ...
    } while (itPreviousParam->getClusterPtr()->getChamberId() ==
             itParam->getClusterPtr()->getChamberId());
    initTrack(*itPreviousParam->getClusterPtr(), *itParam->getClusterPtr(),
              *itParam);
  }
...
//_________________________________________________________________________________________________
void TrackFitter::initTrack(const Cluster &cl1, const Cluster &cl2,
                            TrackParam &param) const {
  /// Compute the initial track parameters at the z position of the last cluster
  /// (cl2) The covariance matrix is computed such that the last cluster is the
  /// only constraint (by assigning an infinite dispersion to the other cluster)
  ...
  double inverseBendingMomentum =
      1. / TrackExtrap::getBendingMomentumFromImpactParam(bendingImpact);
  param.setInverseBendingMomentum(inverseBendingMomentum);

  // compute the track parameter covariances at the last cluster (as if the
  // other clusters did not exist)
  ...
  if (mUseChamberResolution) {
    ...
    lastParamCov(1, 1) =
        (1000. * mChamberResolutionX2 + lastParamCov(0, 0)) / dZ / dZ;
    // Bending plane
  ...
  // Inverse bending momentum (vertex resolution + bending slope resolution +
  // 10% error on dipole parameters+field)
  if (TrackExtrap::isFieldON()) {
    lastParamCov(4, 4) = ((mBendingVertexDispersion2 +
                           (cl1.getZ() * cl1.getZ() * lastParamCov(2, 2) +
                            cl2.getZ() * cl2.getZ() * 1000. * cl1Ey2) /
                               dZ / dZ) /
                              bendingImpact / bendingImpact +
                          0.1 * 0.1) *
                         inverseBendingMomentum * inverseBendingMomentum;
    lastParamCov(2, 4) = cl1.getZ() * lastParamCov(2, 2) *
                         inverseBendingMomentum / bendingImpact / dZ;
    lastParamCov(4, 2) = lastParamCov(2, 4);
    lastParamCov(3, 4) =
        -(cl1.getZ() * lastParamCov(2, 2) + cl2.getZ() * 1000. * cl1Ey2) *
...
//_________________________________________________________________________________________________
void TrackFitter::addCluster(const TrackParam &startingParam, const Cluster &cl,
                             TrackParam &param) {
  /// Extrapolate the starting track parameters to the z position of the new
  /// cluster accounting for MCS dispersion in the current chamber and the
  /// other(s) crossed Recompute the parameters adding the cluster constraint
  /// with the Kalman filter Throw an exception in case of failure
  ...
  while (currentChamber < expectedChamber) {
    if (!TrackExtrap::extrapToZCov(param, SDefaultChamberZ[expectedChamber],
                                   mSmooth)) {
      throw runtime_error("Track extrapolation failed");
    }
    TrackExtrap::addMCSEffect(param, SChamberThicknessInX0[expectedChamber],
                              -1.);
    expectedChamber--;
  }
...
void TrackFitter::smoothTrack(Track &track, bool finalize) {
  ...
  // smoothed parameters and covariances at first cluster = filtered parameters
  // and covariances
  ...
  itPreviousParam->setLocalChi2(itPreviousParam->getTrackChi2() -
                                itCurrentParam->getTrackChi2());
...
void TrackFitter::runKalmanFilter(TrackParam &trackParam) {
  /// Compute the new track parameters including the attached cluster with the
  /// Kalman filter The current parameters are supposed to have been
  /// extrapolated to the cluster z position Throw an exception in case of
  /// failure
  ...
  TMatrixD addChi2Track(tmp, TMatrixD::kTransposeMult,
                        tmp3);                        // ((p'-p)^-1)W(p'-p)
  tmp = newParam;                                     // p'
  tmp -= clusterParam;                                // (p'-m)
  TMatrixD tmp4(clusterWeight, TMatrixD::kMult, tmp); // U(p'-m)
  addChi2Track += TMatrixD(tmp, TMatrixD::kTransposeMult,
                           tmp4); // ((p'-p)^-1)W(p'-p) + ((p'-m)^-1)U(p'-m)
  trackParam.setTrackChi2(trackParam.getTrackChi2() + addChi2Track(0, 0));
}

//_________________________________________________________________________________________________
void TrackFitter::runSmoother(const TrackParam &previousParam,
                              TrackParam &param) {
  /// Recompute the track parameters starting from the previous ones
  /// Throw an exception in case of failure

  // get variables
  const TMatrixD &extrapParameters =
      previousParam.getExtrapParameters();                    // X(k+1 k)
  const TMatrixD &filteredParameters = param.getParameters(); // X(k k)
  const TMatrixD &previousSmoothParameters =
      previousParam.getSmoothParameters();                    // X(k+1 n)
  const TMatrixD &propagator = previousParam.getPropagator(); // F(k)
  const TMatrixD &extrapCovariances =
      previousParam.getExtrapCovariances();                     // C(k+1 k)
  const TMatrixD &filteredCovariances = param.getCovariances(); // C(k k)
  const TMatrixD &previousSmoothCovariances =
      previousParam.getSmoothCovariances(); // C(k+1 n)
  ...
  TMatrixD smootherGain(filteredCovariances, TMatrixD::kMultTranspose,
                        propagator); // C(kk) * F(k)^t
  smootherGain *= extrapWeight;      // C(kk) * F(k)^t * (C(k+1 k))^-1

  // compute smoothed parameters: X(k n) = X(k k) + A(k) * (X(k+1 n) - X(k+1 k))
  TMatrixD tmpParam(previousSmoothParameters, TMatrixD::kMinus,
                    extrapParameters); // X(k+1 n) - X(k+1 k)
  TMatrixD smoothParameters(smootherGain, TMatrixD::kMult,
                            tmpParam); // A(k) * (X(k+1 n) - X(k+1 k))
  smoothParameters +=
      filteredParameters; // X(k k) + A(k) * (X(k+1 n) - X(k+1 k))
  param.setSmoothParameters(smoothParameters);

  // compute smoothed covariances: C(k n) = C(k k) + A(k) * (C(k+1 n) - C(k+1
  // k)) * (A(k))^t
  TMatrixD tmpCov(previousSmoothCovariances, TMatrixD::kMinus,
                  extrapCovariances); // C(k+1 n) - C(k+1 k)
  TMatrixD tmpCov2(tmpCov, TMatrixD::kMultTranspose,
                   smootherGain); // (C(k+1 n) - C(k+1 k)) * (A(k))^t
  TMatrixD smoothCovariances(
      smootherGain, TMatrixD::kMult,
      tmpCov2); // A(k) * (C(k+1 n) - C(k+1 k)) * (A(k))^t
  smoothCovariances +=
      filteredCovariances; // C(k k) + A(k) * (C(k+1 n) - C(k+1 k)) * (A(k))^t
  ...
  TMatrixD tmpChi2(smoothResidual, TMatrixD::kTransposeMult,
                   smoothResidualWeight); // (r(k n))^t * W(k n)
  TMatrixD localChi2(tmpChi2, TMatrixD::kMult,
                     smoothResidual); // (r(k n))^t * W(k n) * r(k n)
