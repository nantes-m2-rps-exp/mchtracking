...
public:
  ...
  /// Return an iterator to the track parameters at clusters (point to the first
  /// one)
  ...
  /// Return a reverse iterator to the track parameters at clusters (point to
  /// the last one)
  ...
  /// Remove the given track parameters from the internal list and return an
  /// iterator to the parameters that follow
  auto removeParamAtCluster(std::list<TrackParam>::iterator &itParam) {
    return mParamAtClusters.erase(itParam);
  }

  int getNClustersInCommon(const Track &track, int stMin = 0,
                           int stMax = 4) const;
  ...
  /// get a reference to the current chamber on which the current parameters are
  /// given
...
private:
  std::list<TrackParam>
      mParamAtClusters{}; ///< list of track parameters at each cluster
  std::unique_ptr<TrackParam>
      mCurrentParam{}; ///< current track parameters used during tracking
  int mCurrentChamber =
      -1; ///< current chamber on which the current parameters are given
  bool mConnected =
      false; ///< flag telling if this track shares cluster(s) with another
