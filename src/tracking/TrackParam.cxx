...
//_________________________________________________________________________
TrackParam::TrackParam(Double_t z, const Double_t param[5],
...
TrackParam::TrackParam(const TrackParam &tp)
    : mZ(tp.mZ), mParameters(tp.mParameters), mClusterPtr(tp.mClusterPtr),
      mRemovable(tp.mRemovable), mTrackChi2(tp.mTrackChi2),
...
Double_t TrackParam::px() const {
  ...
  if (TMath::Abs(mParameters(4, 0)) > 0) {
    ...
    pZ = -pYZ / (TMath::Sqrt(1.0 + mParameters(3, 0) *
                                       mParameters(3, 0))); // spectro. (z<0)
  } else {
    pZ = -FLT_MAX / TMath::Sqrt(1.0 + mParameters(3, 0) * mParameters(3, 0) +
                                mParameters(1, 0) * mParameters(1, 0));
  }
...
Double_t TrackParam::pz() const {
  ...
  if (TMath::Abs(mParameters(4, 0)) > 0) {
    Double_t pYZ = TMath::Abs(1.0 / mParameters(4, 0));
    return -pYZ / (TMath::Sqrt(1.0 + mParameters(3, 0) *
                                         mParameters(3, 0))); // spectro. (z<0)
  } else {
    return -FLT_MAX / TMath::Sqrt(1.0 + mParameters(3, 0) * mParameters(3, 0) +
                                  mParameters(1, 0) * mParameters(1, 0));
  }
}
...
Double_t TrackParam::p() const {
  ...
  if (TMath::Abs(mParameters(4, 0)) > 0) {
    Double_t pYZ = TMath::Abs(1.0 / mParameters(4, 0));
    Double_t pZ =
        -pYZ / (TMath::Sqrt(1.0 + mParameters(3, 0) *
                                      mParameters(3, 0))); // spectro. (z<0)
    return -pZ * TMath::Sqrt(1.0 + mParameters(3, 0) * mParameters(3, 0) +
                             mParameters(1, 0) * mParameters(1, 0));
  } else {
    return FLT_MAX;
  }
...
void TrackParam::setCovariances(const Double_t covariances[15]) {
  ...
  /// <pre> [0] = <X,X> [1] = <SlopeX,X>  [2] = <SlopeX,SlopeX> [3] = <Y,X> [4]
  /// = <Y,SlopeX>       [5] = <Y,Y> [6] = <SlopeY,X>  [7] = <SlopeY,SlopeX> [8]
  /// = <SlopeY,Y>  [9] = <SlopeY,SlopeY> [10]= <q/pYZ,X>   [11]= <q/pYZ,SlopeX>
  /// [12]= <q/pYZ,Y>   [13]= <q/pYZ,SlopeY>   [14]= <q/pYZ,q/pYZ> </pre>
  if (!mCovariances) {
    mCovariances = std::make_unique<TMatrixD>(5, 5);
  }
  for (Int_t i = 0; i < 5; i++) {
    for (Int_t j = 0; j <= i; j++) {
      (*mCovariances)(i, j) = (*mCovariances)(j, i) =
          covariances[i * (i + 1) / 2 + j];
    }
  }
}
...
void TrackParam::setVariances(const Double_t covariances[15]) {
  /// Set the diagonal terms of the covariance matrix (variances) from the
  /// reduced matrix formated as follow: <pre> [0] = <X,X> [1] = <SlopeX,X>  [2]
  /// = <SlopeX,SlopeX> [3] = <Y,X>       [4] = <Y,SlopeX>       [5] = <Y,Y> [6]
  /// = <SlopeY,X>  [7] = <SlopeY,SlopeX>  [8] = <SlopeY,Y>  [9] =
  /// <SlopeY,SlopeY> [10]= <q/pYZ,X>   [11]= <q/pYZ,SlopeX>   [12]= <q/pYZ,Y>
  /// [13]= <q/pYZ,SlopeY>   [14]= <q/pYZ,q/pYZ> </pre>
...
//__________________________________________________________________________
Bool_t TrackParam::isCompatibleTrackParam(const TrackParam &trackParam,
                                          Double_t sigma2Cut,
                                          Double_t &chi2) const {
  /// Return kTRUE if the two set of track parameters are compatible within
  /// sigma2Cut Set chi2 to the compatible chi2 value Note that parameter
  /// covariances must exist for at least one set of parameters Note also that
  /// if parameters are not given at the same Z, results will be meaningless
  ...
  // check Z parameters
  if (mZ != trackParam.mZ) {
    LOG(warn) << "Parameters are given at different Z position (" << mZ << " : "
              << trackParam.mZ << "): results are meaningless";
  }
...
std::ostream &operator<<(std::ostream &out, const TrackParam &param) {
  out << "<TrackParam> Bending P=" << setw(5) << setprecision(3)
      << 1. / param.getParameters()(4, 0) << ", NonBendSlope=" << setw(5)
      << setprecision(3) << param.getParameters()(1, 0) * 180. / TMath::Pi()
      << ", BendSlope=" << setw(5) << setprecision(3)
      << param.getParameters()(3, 0) * 180. / TMath::Pi() << ", (x,y,z)_IP=("
      << setw(5) << setprecision(3) << param.getParameters()(0, 0) << ","
      << setw(5) << setprecision(3) << param.getParameters()(2, 0) << ","
      << setw(5) << setprecision(3) << param.getZ() << ") cm, (px,py,pz)=("
      << setw(5) << setprecision(3) << param.px() << "," << setw(5)
      << setprecision(3) << param.py() << "," << setw(5) << setprecision(3)
      << param.pz() << ") GeV/c, "
      << "local chi2=" << param.getLocalChi2();
  return out;
}
