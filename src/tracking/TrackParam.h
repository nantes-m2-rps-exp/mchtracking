...
public:
  ...
  void setNonBendingCoor(Double_t nonBendingCoor) {
    mParameters(0, 0) = nonBendingCoor;
  }
  ...
  void setNonBendingSlope(Double_t nonBendingSlope) {
    mParameters(1, 0) = nonBendingSlope;
  }
  ...
  void setBendingSlope(Double_t bendingSlope) {
    mParameters(3, 0) = bendingSlope;
  }
  /// return inverse bending momentum (GeV/c ** -1) times the charge (assumed
  /// forward motion)
  Double_t getInverseBendingMomentum() const { return mParameters(4, 0); }
  /// set inverse bending momentum (GeV/c ** -1) times the charge (assumed
  /// forward motion)
  void setInverseBendingMomentum(Double_t inverseBendingMomentum) {
    mParameters(4, 0) = inverseBendingMomentum;
  }
  ...
  void setParameters(const Double_t parameters[5]) {
    mParameters.SetMatrixArray(parameters);
  }
  ...
  /// return true if the associated cluster can be removed from the track it
  /// belongs to
  Bool_t isRemovable() const { return mRemovable; }
  /// set the flag telling whether the associated cluster can be removed from
  /// the track it belongs to or not
  ...
  Bool_t isCompatibleTrackParam(const TrackParam &trackParam,
                                Double_t sigma2Cut, Double_t &normChi2) const;
...
private:
  ...
  /// InvP_yz = Inverse bending momentum (GeV/c ** -1) times the charge (assumed
  /// forward motion)  </pre>
  ...
  /// <X,InvP_yz> <SlopeX,InvP_yz> <Y,InvP_yz> <SlopeY,InvP_yz>
  /// <InvP_yz,InvP_yz>  </pre>
  mutable std::unique_ptr<TMatrixD>
      mCovariances{}; ///< \brief Covariance matrix of track parameters

  /// Jacobian used to extrapolate the track parameters and covariances to the
  /// actual z position
  mutable std::unique_ptr<TMatrixD> mPropagator{};
  /// Track parameters extrapolated to the actual z position (not filtered by
  /// Kalman)
  mutable std::unique_ptr<TMatrixD> mExtrapParameters{};
  /// Covariance matrix extrapolated to the actual z position (not filtered by
  /// Kalman)
  mutable std::unique_ptr<TMatrixD> mExtrapCovariances{};

  mutable std::unique_ptr<TMatrixD>
      mSmoothParameters{}; ///< Track parameters obtained using smoother
  mutable std::unique_ptr<TMatrixD>
      mSmoothCovariances{}; ///< Covariance matrix obtained using smoother

  const Cluster *mClusterPtr =
      nullptr; ///< Pointer to the associated cluster if any

  Bool_t mRemovable = false; ///< kTRUE if the associated cluster can be removed
                             ///< from the track it belongs to

  Double_t mTrackChi2 =
      0.; ///< Chi2 of the track when the associated cluster was attached
  Double_t mLocalChi2 =
      0.; ///< Local chi2 of the associated cluster with respect to the track
};
